...
class DashboardManagement extends Controller
{
    ...
    public function Dashboard()
    {
    	...
    	$engineer = User::select('users.*')->where('role', 'engineer')->count();
        $capacity = User::select('users.*')->where('role', 'capacity')->count();
        ...
        $approve_r = Improvement::select('improvements.*')->where('status', 'approve')->count();
        ...
        ->select('worklists.*', 'lanes.lane as lane_name', 'machines.machine_name', 'machines.brand', 'machines.capacity', 'users.name')
    	...
    	return view('dashboard', compact('user', 'machine', 'annual', 'repair', 'admin', 'engineer', 'users', 'capacity', 'open_a', 'waiting_a', 'close_a', 'open_r', 'waiting_r', 'close_r', 'approve_r', 'count_2', 'open', 'waiting', 'close', 'repair_5', 'annual_4', 'activity_log'));
    }
