...
class ImprovementsManagement extends Controller
{
    ...
    public function TableRepair()
    {
        ...
        ->select('improvements.*', 'lanes.lane as lane_name', 'machines.crq_remedy as crq_remedy')
        ->get();
    ...
    public function CreateImprovement(Request $request)
    {
    	...
    	$repairs->vmname = $request->vmname;
    	$repairs->os= $request->os;
        $repairs->vmname = $request->vmname;
    	$repairs->os= $request->os;
        $repairs->ipaddress = $request->ipaddress;
        ...
        $log->description = $request->user()->name." added a new Reconfigure";
    ...
    public function UpdateImprovement(Request $request, $id)
    {
        ...
        $log->id_user = $request->user()->id;
        $log->user = $request->user()->name;
        $log->description = $request->user()->name." changes the Reconfigure Machine " . $repairs->repair_code;
        $log->remember_token = Str::random(40);
        $log->save();

    	Session::flash('updated', 'Reconfigure successfully updated!');
    	echo "success";
    }
    ...
    public function DeleteImprovement($id)
    {
        ...
        $log->description = Auth::user()->name." deletes the Reconfigure Machine ". $repairs->repair_code;
        $log->remember_token = Str::random(40);
        $log->save();

    	Session::flash('deleted', 'Reconfigure successfully deleted!');
    	return redirect('/my_repair_machine');
    }
    ...
    public function UpdateStatus(Request $request, $id)
    {
        ...
        $log->id_user = $request->user()->id;
        $log->user = $request->user()->name;
        $log->description = $request->user()->name." changes status on the Reconfigure Machine " . $repairs->repair_code;
        $log->remember_token = Str::random(40);
        $log->save();
