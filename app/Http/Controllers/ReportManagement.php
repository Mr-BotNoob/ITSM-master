...
class ReportManagement extends Controller
{
    ...
    public function WorkerHistory($id)
    {
        ...
        ->join('machines', 'machines.machine_code', '=', 'improvements.crq_remedy')
    ...
    {	
        ...
        ->join('machines', 'improvements.crq_remedy', '=', 'machines.machine_code')
        ...
        if($request->report_type == 'all'){
            if($request->status != 'all'){
                if($request->check_history != 'true'){
                    ...
                    ->select('worklists.*', 'machines.machine_name', 'lanes.lane as lane_name', 'machines.brand', 'machines.capacity', 'machines.email')
                    ->get();
                    $repairs = Improvement::join('lanes', 'lanes.id', '=', 'improvements.lane')
                    ->join('machines', 'machines.machine_code', '=', 'improvements.crq_remedy')
                    ->where('improvements.id_user', $id)
                    ...
                    ->select('improvements.*', 'lanes.lane as lane_name', 'machines.brand', 'machines.capacity', 'machines.email', 'machines.machine_name as machine_name')
                    ->get();
                }else{
                    $routines = Worklist::join('annualschedulelists', 'annualschedulelists.schedule_list_code', '=', 'worklists.schedule_list_code')
                    ->join('annualschedules', 'annualschedules.schedule_code', '=', 'annualschedulelists.schedule_code')
    ...
    public function loadReportMachine(Request $request)
    {
        ...
        $log->id_user = Auth::id();
        $log->user = Auth::user()->name;
        $log->description = Auth::user()->name." print a history report";
        $log->remember_token = Str::random(40);
        $log->save();
        ...
        ->where('improvements.crq_remedy', $request->machine)
        ->count();
        ...
        if($worklist_count != 0){
            ...
            ->join('machines', 'machines.machine_code', '=', 'annualschedules.machine_code')
            ->join('lanes', 'lanes.id', '=', 'annualschedules.lane')
            ->select('worklists.*', 'machines.machine_code as code_machine', 'machines.machine_name as machine_name', 'machines.brand', 'machines.capacity', 'machines.email', 'lanes.id as code_lane', 'lanes.lane as lane_name')
            ->where('annualschedulelists.lane', $request->lane)
            ->where('annualschedulelists.machine_code', $request->machine)
            ->first();
        }else if($repairs_count != 0){
            $machine_name = Improvement::join('machines', 'machines.machine_code', '=', 'improvements.crq_remedy')
            ->join('lanes', 'lanes.id', '=', 'improvements.lane')
            ->select('improvements.*', 'machines.machine_code as code_machine', 'machines.machine_name as machine_name', 'machines.brand', 'machines.capacity', 'lanes.id as code_lane', 'lanes.lane as lane_name')
            ->where('improvements.lane', $request->lane)
            ->where('improvements.crq_remedy', $request->machine)
            ->first();
        }else{
        ...
        if($status != 'all')
        {
            if($request->check_history != 'true')
            {
                ...
                ->distinct()
                ->get();
                $repairs = Improvement::select('improvements.vmname')
                ->where('improvements.lane', $request->lane)
                ->where('improvements.crq_remedy', $request->machine)
    ...
    public function checkLeftJoin()
    {
        ...
        ->where('improvements.crq_remedy', 'M0001')
        ->first();
        $repairs_min_date = Improvement::select(DB::raw('min(improvements.repair_date) as min_date'))
        ->where('improvements.lane', '1')
        ->where('improvements.crq_remedy', 'M0001')
